...
E1 = E/(1-(v^2))*(10^-3); % E* from cell modulus (10^3) in MPa.
...
for i = 1:size(D_cell_filename,1) 
  ...
  %Fit power law function to strain_rate vs stress
        [fitVisco_interval, gof_interval] = create_vpmodel(strain_rate, stress);
        fitVisco_interval_coeff = coeffvalues(fitVisco_interval);   % fitting coefficients
    ...
    prony(i,11) = fitVisco_interval_coeff(1);
    prony(i,12) = fitVisco_interval_coeff(2);
    prony(i,13) = fitVisco_interval_coeff(3);
    prony(i,14) = gof_interval_number(2);

    stress_fit =(fitVisco_interval_coeff(1)*(strain_rate.^fitVisco_interval_coeff(2)))+ fitVisco_interval_coeff(3);

    figure(1)
    plot(strain_rate,stress,'bo')
    hold on
    plot(strain_rate, stress_fit, '-r')
    hold off
   ...
   vp_model(:,3) = prony(:,13); %yield stress
   vp_model(:,4) = prony(:,9); %H0
   vp_model(:,5) = prony(:,10); %Heq
   vp_model(:,6) = prony(:,14); %R2
